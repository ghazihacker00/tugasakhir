...
@section('title', 'Kelola API TTE Requests')

@section('content')
<div class="bg-gray-100 shadow-lg rounded-xl p-8 transition duration-500 ease-in-out transform">
    <div class="flex justify-between items-center mb-8">
        <h2 class="text-4xl font-extrabold text-gray-900">Pengajuan Layanan Integrasi Sertifikat Elektronik dan Sistem Elektronik</h2>
    </div>
    ...
    <!-- Tabel Pengajuan API TTE Requests -->
            ...
            <tbody id="apiTTETableBody" class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-100 transition duration-300 ease-in-out transform">
                    ...
                    <td class="py-4 px-6 text-center">
                        <form action="{{ route('admin.api-tte.updateStatus', $request->id) }}" method="POST">
                            @csrf
                    ...
                    <td class="py-4 px-6 text-center">
                        <div class="flex justify-center items-center space-x-4">
                            <a href="{{ route('admin.api-tte.edit', $request->id) }}" class="text-green-500 hover:text-green-700 transition duration-300">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <form action="{{ route('admin.api-tte.destroy', $request->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
...
<script>
    ...
    document.getElementById('searchInput').addEventListener('input', function () {
        fetch(`{{ route('admin.api-tte.index') }}?search=${this.value}`)
