...
@section('title', 'Kelola E-Sign Requests')
...
<div class="bg-gray-100 shadow-lg rounded-xl p-8 transition duration-500 ease-in-out transform">
    ...
    <!-- Tabel Pengajuan E-Sign Requests -->
    <div class="overflow-x-auto rounded-lg shadow-lg bg-white">
        <table class="min-w-full">
            ...
            <tbody id="eSignTableBody" class="bg-white divide-y divide-gray-200">
                @foreach ($eSignRequests as $request)
                <tr class="hover:bg-gray-100 transition duration-300 ease-in-out transform">
                    ...
                    <td class="py-4 px-6 text-center">
                        <form action="{{ route('admin.e-sign.updateStatus', $request->id) }}" method="POST">
                            @csrf
                    ...
                    <td class="py-4 px-6 text-center">
                        ...
                        <a href="{{ Storage::url($request->surat_permohonan) }}" download class="text-indigo-600 hover:text-indigo-800 font-semibold transition duration-300">Download</a>
                        @else
                    ...
                    <td class="py-4 px-6 text-center">
                        <div class="flex justify-center items-center space-x-4">
                            <a href="{{ route('admin.e-sign.edit', $request->id) }}" class="text-green-500 hover:text-green-700 transition duration-300">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <form action="{{ route('admin.e-sign.destroy', $request->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
    ...
    <!-- Paginasi -->
    <div class="mt-8">
        {{ $eSignRequests->links('pagination::simple-tailwind') }}
    </div>
</div>
...
<script>
    ...
    document.getElementById('searchInput').addEventListener('input', function () {
        fetch(`{{ route('admin.e-sign.index') }}?search=${this.value}`)
            .then(response => response.text())
            .then(html => {
                const parser = new DOMParser();
                const doc = parser.parseFromString(html, 'text/html');
                const newTbody = doc.querySelector('#eSignTableBody');
                document.getElementById('eSignTableBody').replaceWith(newTbody);
            });
    });
